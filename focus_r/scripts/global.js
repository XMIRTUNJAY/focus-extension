...
var defaultBlockPage = "https://www.bumblebeesystems.com/wastenotime/block.html?";
...
function getRedirectURL( a ) {
    return ( settings.blockurlUseDefault ? ( defaultBlockPage + a ) : cleanUpURL( settings.blockurl ) )
}
...
function openPage( a ) { myextension.openTab( a ) }
...
function performCommand( a ) { if ( a.command === "add-site" ) { addSiteCommand() } else { if ( a.command === "add-entire-site" ) { addEntireSiteCommand() } else { if ( a.command === "show-settings" ) { showSettings() } } } } function configChanged( a ) {
...
function respondToMessage( b, e, c, a ) {
    if ( b === "checkURL" ) {
        ...
        if ( SAFARI ) {
            ...
            if ( DEBUG ) { window.console.log( "Add tab id [" + c.tab.id + "] => url:" + d.url + " referrer:" + d.referrer ) } chrome_referrers[ c.tab.id ] = d.referrer;

            if ( shouldBlock( d.url, d.referrer, myextension.isTabIncognito( c.tab ), function ( g ) { alert( g ); return true } ) ) { chrome.tabs.update( c.tab.id, { url: getRedirectURL( d.url ) } ) }
        }
        else {
            if ( shouldBlock( d.url, d.referrer, myextension.isTabIncognito( c ),

                function ( g ) { a( "Alert", g ); return true } ) ) { c.url = getRedirectURL( d.url ) } isIdle = ( d.idleTime >= settings.idleTimeout ); updateToolbarIcon()
        } updatePopover()
    } else {
        if ( b === "reload" ) {
            if ( e ) {
